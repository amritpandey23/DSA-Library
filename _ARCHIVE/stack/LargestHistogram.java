...
public class LargestHistogram {
    ...
    public static void main(String[] args) {
        // code here
        int[] A = { 6, 2, 5, 4, 1, 5, 6 };
        System.out.println(largestHistogramArea1(A));
    }

    public static int largestHistogramArea1(int[] A) {
        ...
        for (int i = 0; i < A.length; i++) {
            int area = A[i];
            area += (ps[i] == -1 ? 0 : (i - ns[i] - 1)) * A[i];
            area += (ns[i] == -1 ? 0 : (ns[i] - i - 1)) * A[i];
    ...
    public static int[] previousSmaller(int[] A) {
        ...
        for (int i = 1; i < A.length; i++) {
            ...
            int s = stack.size() == 0 ? -1 : stack.peek();
            ps[i] = s;
            stack.push(i);
        }
        return ps;
