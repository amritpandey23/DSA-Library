...
public class DFS {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(performDFS(graph, 0, 4, visited));
    }

    public static boolean performDFS(ArrayList<Integer>[] graph, int src, int dest, boolean[] visited) {
        if (src == dest) {
            return true;
        }
        visited[src] = true;
        for (int nbr : graph[src]) {
            if (!visited[nbr]) {
                if (performDFS(graph, nbr, dest, visited)) {
                    return true;
                }
            }
        }
