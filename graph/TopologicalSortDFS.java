...
public class TopologicalSortDFS {
    ...
    public static void topologicalSort(List<Integer>[] graph) {
        ...
        for (int vtx = 0; vtx < V; vtx++) {
            if (!visited[vtx]) {
                topologicalSort(graph, vtx, Stk, visited);
            }
        }
        while (!Stk.isEmpty()) {
    ...
    public static void topologicalSort(List<Integer>[] graph, int src, ArrayDeque<Integer> Stk, boolean[] visited) {
        visited[src] = true;
        for (int nbr : graph[src]) {
            if (!visited[nbr]) {
                topologicalSort(graph, nbr, Stk, visited);
            }
        }
        Stk.push(src);
    }
