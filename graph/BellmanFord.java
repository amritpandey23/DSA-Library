...
public class BellmanFord {
    public static void main(String[] args) {
        // code here
        int[][] edg = {{0, 1, 2}, {2, 0, 1}};
        Edge[] edges = new Edge[edg.length];
        for (int i = 0; i < edg.length; ++i) {
            edges[i] = new Edge(edg[i][0], edg[i][1], edg[i][2]);
        }
        int V = 3;
        bellmanFord(edges, V);
    }

    public static void bellmanFord(Edge[] edges, int V) {
        int[] dist = new int[V];
        ...
        for (int count = 0; count < V - 1; ++count) {
            for (Edge e : edges) {
                if (dist[e.src] != Integer.MAX_VALUE) {
                    dist[e.dest] = Math.min(dist[e.dest], dist[e.src] + e.wt);
                }
            }
        }
        for (Edge e : edges) {
