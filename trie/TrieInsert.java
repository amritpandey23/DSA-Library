...
public class TrieInsert {
    ...
    public static void main(String[] args) {
        ...
        insert("abhinav", dict);
        insert("amit", dict);
        System.out.println(search("amrit", dict));
        System.out.println(search("abhinav", dict));
        System.out.println(search("amit", dict));
        System.out.println(search("vaibhav", dict));
        System.out.println(search("anish", dict));
    }

    public static boolean search(String word, TrieNode root) {
        TrieNode curr = root;
        for (int c = 0; c < word.length() && curr != null; c++) {
            curr = curr.childs[word.charAt(c)];
        }
        return curr == null ? false : curr.isLast;
    }

    public static void insert(String word, TrieNode root) {
        TrieNode curr = root;
        for (int c = 0; c < word.length(); c++) {
            if (curr.childs[word.charAt(c)] == null) {
                curr.childs[word.charAt(c)] = new TrieNode();
            }
            curr = curr.childs[word.charAt(c)];
            if (c == word.length() - 1) {
                curr.isLast = true;
